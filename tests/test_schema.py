...
def test_timeframe_columns_are_case_sensitive():
    ...
    assert schema.count("timeframe VARCHAR(8) CHARACTER SET ascii COLLATE ascii_bin NOT NULL") == 5
    ...
    assert "source_market VARCHAR(48) NOT NULL DEFAULT ''" in schema
    assert "asset_class VARCHAR(24) NOT NULL DEFAULT ''" in schema
    assert "bar_history_adjusted" in schema
    assert "adjustment_factor" in schema
    assert "instrument_change_event" in schema
